...
namespace o2scl {
  ...
  */
  template<class param_t, class param_vec_t=std::vector<param_t>,
    class func_t=multi_funct<param_t>, class vec_t=ubvector,
    class alloc_vec_t=ubvector, class alloc_t=ubvector_alloc,
    class rng_t=rng_gsl> class anneal_mt {
  ...
  */
	...
	for(size_t ip=0;ip<np;ip++) {
	  ...
	  if(new_E[ip]<=best_E){
	    ...
	    if (this->verbose>0 && out_best) {
	      std::cout << "Best: " << best_x << " " << best_E << std::endl;
	    }
	  }
	  ...
	  if (new_E[ip]<E) {
	    ...
	    E=new_E[ip];
	    nmoves++;
	  } else if (this->def_rng.random() < exp(-(new_E[ip]-E)/(boltz*T)) ) {
	    for(j=0;j<nv;j++) x[j]=new_x[ip][j];
	    E=new_E[ip];
	    nmoves++;
	  }
  ...
  virtual int next(size_t nv, vec_t &x_old, double min_old, vec_t &x_new, 
	...
	std::cout << "Step sizes changed: " << step_vec << std::endl;
      }
    }
    T/=T_dec;
    return gsl_success;
  ...
  */
			 ...
			 double tptr, std::string comment) 
  {
    ...
    text_out_file outsf(this->outs,79);
    outsf.word_out("x:");
    for(i=0;i<nv;i++) outsf.double_out(xx[i]);
    outsf.end_line();
  ...
  void func_wrapper(size_t ip) {
    step(nvar,new_x[ip]);
    (*f)(nvar,new_x[ip],new_E[ip],(*params)[ip]);
  }
  ...
  //@{
  alloc_vec_t x, *new_x, best_x, new_E, old_x;
  //@}
  ...
  */
  virtual int allocate(size_t nv, size_t np) {
    ...
    nproc=np;
    ao.allocate(x,nvar);
    new_x=new alloc_vec_t[np];
    ao.allocate(old_x,nvar);
    for(size_t i=0;i<np;i++) {
      ao.allocate(new_x[i],nvar);
    }
    ao.allocate(new_E,np);
    ao.allocate(best_x,nvar);
    return 0;
  }
