...
/*
  This compares the speed of integrating ODE's with the Prince-Dormand
  stepper using various approaches. Using arrays is typically faster
  than using ovector objects, and the class gsl_rk8pd_fast is faster
  than gsl_rk8pd.  Note the relative performance will change somewhat
...
int derivs2_arr(double x, size_t nv, const double y[], 
		double dydx[], int &pa) {
  dydx[0]=y[0];
  dydx[1]=y[1];
  return 0;
...
int main(void) {
  ...
  gsl_rk8pd<int,ode_funct_fptr<int >,ovector_base,ovector,
    ovector_alloc> rk;
  gsl_rk8pd_fast<1,int,ode_funct_fptr<int >,ovector_base,ovector,
    ovector_alloc> rkf;
  gsl_rk8pd<int,ode_vfunct_fptr<int,1>,
    double [1],double [1],array_alloc<double [1]> > rka;
  gsl_rk8pd_fast<1,int,ode_vfunct_fptr<int,1>,
    double [1],double [1],array_alloc<double [1]> > rkaf;

  test_mgr t;
  t.set_output_level(1);
  size_t tmp, t1=0, t2=0, t3=0, t4=0, t5=0, t6=0;
  ...
  for(size_t ii=0;ii<5;ii++) {
    ...
    gsl_odeiv_step *s=gsl_odeiv_step_alloc(gsl_odeiv_step_rk8pd,1);
    ...
    // Arrays with gsl_rk8pd_fast and direct function call
  
    typedef int (*der_fn_t)(double x, size_t nv, const double y[], 
			    double dydx[], int &pa);
    der_fn_t fnp=&derivs_arr;
    gsl_rk8pd_fast<1,int,der_fn_t,
