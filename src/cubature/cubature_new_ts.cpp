...
int fv_new(size_t ndim, size_t npt, c_ubvector_range &x, size_t fdim,
	   ubvector_range &fval) {
  ...
  for (size_t i=0;i<npt;i++) {
    if (f_test(ndim,x2+i*ndim,fdim,fv2+i*fdim)) {
      return o2scl::gsl_failure;
    }
  }
  delete[] x2;
...
int fv2_new(size_t ndim, size_t npt, c_ubvector_range &x, size_t fdim,
	    ubvector_range &fval) {

  for (size_t i=0;i<npt;i++) {
    c_ubvector_range_range x2=vector_range(x,i*ndim,(i+1)*ndim);
...
int main(void) {
  ...
  typedef std::function<
    int(size_t,size_t,c_ubvector_range &,size_t,ubvector_range &)>
    cub_funct_ub;
  inte_hcubature<cub_funct_arr> hc;
  inte_pcubature<cub_funct_ub,ubvector,c_ubvector_range,
		 ubvector_range> pc;

  inte_cubature_base::error_norm en=inte_cubature_base::ERROR_INDIVIDUAL;
  inte_cubature_base::error_norm en2=inte_cubature_base::ERROR_L2;
  ...
  for(size_t test_iand=0;test_iand<8;test_iand++) {
    ...
    if (test_iand!=3) {
      ...
      cub_count=0;
      pc.integ(1,cfa_new,dim,xmin,xmax,maxEval,0,tol,en,vval,verr);
  ...
  for(size_t test_iand=0;test_iand<8;test_iand++) {
    ...
    if (test_iand!=2) {
      ...
      cub_count=0;
      hc.integ(1,cfa,dim,xmin,xmax,maxEval,0,tol,en2,vval,verr);
    ...
    if (test_iand!=3) {
      ...
      cub_count=0;
      pc.integ(1,cfa_new,dim,xmin,xmax,maxEval,0,tol,en2,vval,verr);
  ...
  
  {
    ...
    ret=pc.integ(3,cfa2_new,2,vlow2,vhigh2,10000,0.0,1.0e-4,en,dres2,derr2);
