...
namespace o2scl {
  ...
  */
  int compute_cacheval(const std::vector<size_t> &m, size_t mi, vec_t &val, 
	...
	buf[k+ibuf*dim]=p[k];
      ...
      if (ibuf == nbuf) {
	/* flush buffer */
	vec_crange_t buf2=o2scl::vector_range
  ...
  /** \brief Desc
   */
  int add_cacheval(std::vector<cache> &vc,
		   const std::vector<size_t> &m, size_t mi, size_t fdim,
		   func_t &f, size_t dim, const vec_t &xmin,
		   const vec_t &xmax, vec_t &buf, size_t nbuf) {
    ...
    if (ibuf > 0) {
      ...
      vec_crange_t buf2=o2scl::vector_range(((const vec_t &)buf),0,buf.size());
  ...
  */
  void eval_integral(std::vector<cache> &vc,
