...
/* Adaptive multidimensional integration on hypercubes (or, really,
   ...
   scratch as part of a larger code-cleanup, and in order to have a
   single code path for the vectorized and non-vectorized APIs. I
   subsequently implemented the algorithm by Gladwell to extract even
   more parallelism by evalutating many hypercubes at once.)
...
(rule *r_, unsigned fdim, integrand_v f, void *fdata, unsigned nR, region *R)
{
