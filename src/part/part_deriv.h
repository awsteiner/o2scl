...
namespace o2scl {
  ...
  */
  ...
  public:
    ...
    /// Copy constructor
    fermion_deriv_tl(const fermion_tl<fp_t> &p) {
      this->g=p.g;
      this->m=p.m;
      this->ms=p.ms;
      this->n=p.n;
    ...
    /// Copy construction with operator=()
    fermion_deriv_tl &operator=(const fermion_tl<fp_t> &p) {
      if (this!=&p) {
	this->g=p.g;
	this->m=p.m;
	this->ms=p.ms;
  ...
  public:
    ...
    /// Make a particle of mass \c mass and degeneracy \c dof.
    part_deriv_tl(fp_t mass=0.0, fp_t dof=0.0) : part_tl<fp_t>(mass,dof) {
    }
  ...
  public:
    ...
    */
    virtual bool calc_mu_deg(fermion_deriv_t &f, fp_t temper,
      ...
      if (fermion_calc_mu_deg(f,temper,prec)==false) {
	return false;
      }
