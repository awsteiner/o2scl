/* ───────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────────
*/
#ifndef O2SCL_FERMION_REL2_H
#define O2SCL_FERMION_REL2_H

/** \file fermion_rel2.h
    \brief File defining \ref o2scl::fermion_rel2_tl
*/
...
namespace o2scl {
  ...
  template<class fp_t> class fermion_rel2_integ {
  ...
  public:
    ...
    */
	...
	O2SCL_ERR2("Returned not finite result ",
		   "in fermion_rel2::deg_density_fun().",exc_einval);
      }
      return ret;
    }
    ...
    internal_fp_t deg_energy_fun(internal_fp_t k, fp_t T2,
      ...
      if (!isfinite(ret)) {
	O2SCL_ERR2("Returned not finite result ",
		   "in fermion_rel2::deg_energy_fun().",exc_einval);
      }
    ...
    internal_fp_t deg_pressure_fun(internal_fp_t k, fp_t T2,
      ...
      if (!isfinite(ret)) {
        return 0.0;
	//O2SCL_ERR2("Returned not finite result ",
        //"in fermion_rel2::deg_pressure_fun().",exc_einval);
      }
	...
	//ret=-k*k*(nx*log(nx)+arg2*log(arg2));
      ...
      if (!isfinite(ret)) {
	O2SCL_ERR2("Returned not finite result ",
		   "in fermion_rel2::deg_entropy_fun().",exc_einval);
      }
  ...
  /** \brief Equation of state for a relativistic fermion
      ...
      fermion_rel2::upper_limit_fac . In the case where \ref
      ...
      using \code fermion_rel2 rf(1.0,2.0); rf.upper_limit_fac=40.0;
  ...
  */
  ...
  class fermion_rel2_tl :
    public fermion_thermo_tl<fermion_t,fd_inte_t,be_inte_t,root_t,
			     func_t,fp_t>, fermion_rel2_integ<fp_t> {
