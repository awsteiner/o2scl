...
void interp2_direct::set_data(size_t n_x, size_t n_y, ubvector &x_grid,
  ...
  if (interp_type==itp_cspline || interp_type==itp_cspline_peri) {
    ...
    for(size_t j=0;j<n_y;j++) {
      ubmatrix_column colt(data,j);
      ubvector col=colt;
      interp_vec<ubvector> itp(n_x,x_grid,col,interp_type);
    ...
    for(size_t i=0;i<n_x;i++) {
      ubmatrix_row rowt(data,i);
      ubvector row=rowt;
      interp_vec<ubvector> itp(n_y,y_grid,row,interp_type);
      for(size_t j=0;j<n_y;j++) {
