...
namespace o2scl {
   ...
   */
  template<class vec_t> class mcovar_funct {
  ...
  public:
    ...
    /// The function
    virtual double operator()(vec_t &x, vec_t &y)=0;

    /// The function
    virtual double deriv(vec_t &x1, vec_t &x2, size_t ix);
    
  };
  
  /** \brief Covariance function: 1D RBF with a noise term
   */
  template<class vec_t> class mcovar_funct_rbf_noise :
    public mcovar_funct<vec_t> {
  ...
  public:
    ...
    virtual double operator()(vec_t &x1, vec_t &x2) {
      double sum=0.0;
      bool equal=true;
      for(size_t j=0;j<len.size();j++) {
