  ...
  public:
    ...
    */
    template<class vec2_t> void set_scales(size_t n, vec2_t &v) {
      if (n==0) {
	O2SCL_ERR("Scale vector size cannot be zero in interpm_krige.",
		  o2scl::exc_einval);
      }
      for(size_t i=0;i<n;i++) {
	if (v[i]<=0.0) {
	  O2SCL_ERR("Scale must be positive and non-zero in interpm_krige.",
		    o2scl::exc_einval);
	}
      }
      scales.resize(n);
    ...
    */
    template<class vec2_t> void eval_err(const vec2_t &x, double &val,
      ...
      if (data_set==false) {
	O2SCL_ERR("Data not set in interpm_krige::eval_err().",
		  exc_einval);
      }
    ...
    */
	...
	std::vector<size_t> >(dists,order,index);
      if (verbose>0) {
	for(size_t i=0;i<order;i++) {
	  std::cout << "interpm_krige: closest point: ";
	  for(size_t k=0;k<nd_in;k++) {
	...
	if (verbose>0) {
	  std::cout << "interpm_krige: distance zero. "
	...
	norm+=1.0/dists[index[i]];
      ...
      if (verbose>0) {
	std::cout << "interpm_krige: norm is " << norm << std::endl;
      }
	...
	for(size_t i=0;i<order;i++) {
	  ...
	  if (verbose>0) {
	    std::cout << "interpm_krige: j,order,value,1/dist: "
		      << j << " " << i << " "
	...
	y[j]/=norm;
	if (verbose>0) {
	  std::cout << "interpm_krige: y[" << j << "]: " << y[j]
		    << std::endl;
	}
      }
    ...
    */
    template<class vec3_t>
      ...
      if (data_set==false) {
	O2SCL_ERR("Data not set in interpm_krige::derivs_err().",
		  exc_einval);
      }
