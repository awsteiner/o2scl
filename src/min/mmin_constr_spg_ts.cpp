...
int quadratic_Hv(size_t nv, const ubvector &x, const ubvector &v,
		 ubvector &hv) {
  for(size_t i=0;i<nv;i++) {
    hv[i]=2.0*(i+1.0)*v[i];
  }
...
int main(void) {
  ...
  multi_funct_fptr<> mff(quadratic);
  grad_funct_fptr<> gff(quadratic_df);
  ool_hfunct_fptr<> ohf(quadratic_Hv);
  
  mmin_constr_spg<multi_funct_fptr<>, grad_funct_fptr<>,ubvector> omp;
  ...
  t.test_gen(omp.last_ntrial==50,"Num iters.");
  t.test_rel(x[0],2.75002e-6,1.0e-4,"result");
