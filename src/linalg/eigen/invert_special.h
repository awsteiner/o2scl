...
namespace o2scl_linalg {
  ...
  */
  ...
  /** \brief Eigen inverse of symmetric positive definite matrix

      This class is only defined if Eigen support was enabled during
      installation.
  */
  template<class eigen_mat_t> class matrix_invert_det_sympd_eigen : 
  ...
  public:
    ...
    /// Invert matrix \c A, returning the inverse in \c A_inv
    virtual int invert(size_t n, const eigen_mat_t &A, eigen_mat_t &A_inv) {
      A_inv=A.llt().solve(Eigen::MatrixXd::Identity(n,n));
      return 0;
    }
    ...
    */
    virtual int invert_det(size_t n, const eigen_mat_t &A,
                           eigen_mat_t &A_inv, double &A_det) {
      Eigen::LLT<eigen_mat_t> llt(A);
      A_inv=llt.solve(Eigen::MatrixXd::Identity(n, n));
      double sqrt_det=llt.matrixL().toDenseMatrix().diagonal().prod();
    ...
    */
    ...
    /// Inver matrix \c A in place
    virtual int invert_inplace(size_t n, eigen_mat_t &A) {
      A=A.llt().solve(Eigen::MatrixXd::Identity(n,n));
      return 0;
    }
