...
namespace o2scl_linalg {
  ...
  */
  template<class mat_t, class vec_t, class vec2_t, class fp_t>
  ...
  */
  template<class mat1_t, class mat2_t, class mat3_t, class vec_t,
    ...
    for(i=0;i<M;i++) {
      for(j=0;j<M;j++) {
	if (i==j) O2SCL_IX2(Q,i,j)=1 ;
	else O2SCL_IX2(Q,i,j)=0 ;
      }
    }
  ...
  */
  template<class mat_t, class vec_t, class vec2_t, class fp_t>
    void QR_svx(size_t M, size_t N, const mat_t &QR, const vec_t &tau, 
	       vec2_t &x) {
    QR_QTvec<mat_t,vec_t,vec2_t,fp_t>(M,N,QR,tau,x);
    o2scl_cblas::trsv<mat_t,vec2_t,fp_t>
      (o2scl_cblas::o2cblas_RowMajor,o2scl_cblas::o2cblas_Upper,
       o2scl_cblas::o2cblas_NoTrans,o2scl_cblas::o2cblas_NonUnit,
       M,N,QR,x);
    return;
  }
  ...
  */
  template<class mat_t, class vec_t, class vec2_t, class vec3_t,
    ...
    QR_svx<mat_t,vec_t,vec2_t,fp_t>(N,N,QR,tau,x);
    return;
  }
  ...
  */
  template<class mat1_t, class mat2_t, class vec1_t, class vec2_t,
    ...
    */
    ...
    for (k=(int)(M-1); k > 0; k--) {
      ...
      fp_t c, s;
      fp_t wk=O2SCL_IX(w,k);
      fp_t wkm1=O2SCL_IX(w,k-1);
    ...
    for (j=0; j < N; j++) {
      fp_t r0j=O2SCL_IX2(R,0,j);
      fp_t vj=O2SCL_IX(v,j);
    ...
    for (k=1;k<kmax;k++) {
      ...
      fp_t c, s;
      fp_t diag=O2SCL_IX2(R,k-1,k-1);
      fp_t offdiag=O2SCL_IX2(R,k,k-1);
   ...
   */
  template<class mat_t, class mat2_t, class mat3_t, class fp_t>
  void QR_decomp_unpack(const size_t M, const size_t N,
                        mat_t &A, mat2_t &Q, mat3_t &R) {
    
    boost::numeric::ublas::vector<fp_t> tau(M);
    QR_decomp<mat_t,boost::numeric::ublas::vector<fp_t>,fp_t>(M,N,A,tau);
    QR_unpack<mat_t,mat2_t,mat3_t,
              boost::numeric::ublas::vector<fp_t>,fp_t>(M,N,A,tau,Q,R);
