...
public:
  ...
  double lefte(size_t ieq, double x, Eigen::VectorXd &yleft) {
    return yleft[1]-1.0;
  }
  
  double righte(size_t ieq, double x, Eigen::VectorXd &yright) {
    return yright[0]-2.0;
  }
  
  double derivse(size_t ieq, double x, Eigen::VectorXd &y) {
    if (ieq==0) return y[0]+y[1];
    return y[0];
  }
