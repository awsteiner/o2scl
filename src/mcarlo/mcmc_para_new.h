...
namespace o2scl {
  ...
  */
  ...
  /** \brief Current points in parameter space for each walker and 
      ...
      This is an array of size \ref n_threads times \ref
      n_walk_per_thread, indexed by
      <tt>thread_index*n_walk_per_threads+walker_index</tt> .
  */
  ...
  /** \brief Data array
      ...
      n_walk_per_thread . The two copies of data objects are indexed by
  ...
  */
  ...
  virtual void mcmc_cleanup() {
    if (verbose>0) {
      for(size_t ic=0;ic<n_chains_per_rank;ic++) {
	scr_out << "mcmc (" << ic << "," << mpi_rank
	<< "): accept=" << n_accept[ic]
	<< " reject=" << n_reject[ic] << std::endl;
      }
      scr_out.close();
    }
    return;
  ...
  /** \brief Index of the current walker for each thread
      ...
      This vector has a size equal to n_threads. The values
      in this vector are between 0 and <tt>n_walk_per_thread-1</tt>.
  */
  std::vector<size_t> curr_walker;

  /** \brief Number of fully independent chains in each MPI rank

      When \ref aff_inv is false, this is always equal to the number
      of threads.
   */
  ...
  */
  ...
  /** \brief Number of walkers per thread (default 0 to set equal to 
      \ref n_walk)
   */
  ...
  */
  virtual int mcmc(size_t n_params, vec_t &low, vec_t &high,
	...
	n_walk=2*n_params;
      ...
      if (n_walk_per_thread==0) {
	std::cout << "mcmc_para::mcmc(): Automatically setting "
		  << "n_walk_per_thread to " << n_walk << "." << std::endl;
	...
	O2SCL_ERR2("More walkers per thread than total walkers ",
      ...
      /* Automatically compute n_chains_per_rank
	 
	 The number of chains in each MPI rank is n_chains_per_rank and
	 the number of walkers in each OpenMP thread is
	 n_walk_per_thread. Thus we heave:
	 
	 n_chains_per_rank * n_walk = n_walk_per_thread * n_threads
	 
	 The value of n_walk should always greater than or equal to
	 n_walk_per_thread, thus n_threads should always be larger
	 than n_chains_per_rank.

	 The idea is that there are n_chains_per_rank=2 spread out over 
	 n_threads=4 with n_walk=60 so that there are 30 walkers per
	 thread. In this case, the initial point evaluation needs
	 to construct 120 initial points, 60 walkers for each chain.
	 Thus each thread computes n_walk_per_thread points.

	 The storage size is then 120.

	 For the main loop, each thread needs to move 30 
	 (n_walk_per_thread) walkers.
      */
	...
	O2SCL_ERR2("Values of n_walk, n_threads, and n_walk_per_thread ",
		   "not commensurate in mcmc_para::mcmc().",
		   o2scl::exc_einval);
      }
      
    } else {

      // If we're not doing affine-invariant sampling
      n_walk_per_thread=n_walk;
...
#pragma omp for
	...
	for(size_t it=0;it<n_threads;it++) {
	  ...
	  for(curr_walker[it]=0;curr_walker[it]<n_walk_per_thread &&
	    ...
	    // Index in storage
	    size_t sindex=n_walk_per_thread*it+curr_walker[it];
...
#pragma omp critical (o2scl_mcmc_para_best_point)
	...
	// The z parameter for each thread
...
#pragma omp for
	  ...
	  for(size_t it=0;it<n_threads;it++) {
	    ...
	    curr_walker[it]=mcmc_iters % n_walk_per_thread;
	    ...
	    // If the selected walker is greater than
	    // n_walk_per_thread, then shift the thread index
	    // and then adjust ij
	    size_t jt=(it+ij/n_walk_per_thread) % n_threads;
	    ij=ij % n_walk_per_thread;
	    
	    // Create new trial point
	    for(size_t i=0;i<n_params;i++) {
	      next[it][i]=current[n_walk_per_thread*jt+ij][i]+
		smove_z[it]*(current[n_walk_per_thread*jt+curr_walker[it]][i]-
			     current[n_walk_per_thread*jt+ij][i]);
	    }
	    ...
	    // necessary, and update the return value array
	    if (func_ret[it]!=mcmc_skip) {
	      if (switch_arr[n_walk_per_thread*it+curr_walker[it]]==false) {
		func_ret[it]=func[it]
		  (n_params,next[it],w_next[it],
		   data_arr[it*n_walk_per_thread+curr_walker[it]+
			    n_walk_per_thread*n_threads]);
	      } else {
		func_ret[it]=func[it]
		  (n_params,next[it],w_next[it],
		   data_arr[it*n_walk_per_thread+curr_walker[it]]);
...
#pragma omp for
	  ...
	  for(size_t it=0;it<n_threads;it++) {
		...
		if (switch_arr[sindex]==false) {
		  meas_ret[it]=meas[it]
		    (next[it],w_next[it],curr_walker[it],func_ret[it],true,
		     data_arr[sindex+n_threads*n_walk_per_thread]);
		} else {
		  meas_ret[it]=meas[it](next[it],w_next[it],
					curr_walker[it],func_ret[it],true,
					data_arr[sindex]);
		...
		if (switch_arr[sindex]==false) {
		  meas_ret[it]=meas[it]
		    (next[it],w_next[it],curr_walker[it],func_ret[it],false,
		     data_arr[sindex+n_threads*n_walk_per_thread]);
		} else {
		  meas_ret[it]=meas[it](next[it],w_next[it],
					curr_walker[it],func_ret[it],false,
	  ...
	  if (func_ret[it]==o2scl::success && w_best>w_next[it]) {
	    ...
	    if (switch_arr[n_walk_per_thread*it+curr_walker[it]]==false) {
	      best_point(best,w_best,
			 data_arr[curr_walker[it]+n_walk_per_thread*it+
				  n_threads*n_walk_per_thread]);
	    } else {
	      best_point(best,w_best,
			 data_arr[curr_walker[it]+n_walk_per_thread*it]);
	    }
