...
class mcmc_para_new_class {

public:

  int count;
  
  mcmc_para_new_base<point_funct,measure_funct,std::array<double,1>,
                     ubvector> mc;

  mcmc_para_new_table<point_funct,fill_funct,std::array<double,1>,
                      ubvector> mct;
...
int main(int argc, char *argv[]) {
  ...
  multi_funct mf0=std::bind
    (std::mem_fn<double(size_t,const ubvector &)>(&mcmc_para_new_class::f0),
     &mpc,std::placeholders::_1,std::placeholders::_2);
  multi_funct mf1=std::bind
    (std::mem_fn<double(size_t,const ubvector &)>(&mcmc_para_new_class::f1),
     &mpc,std::placeholders::_1,std::placeholders::_2);
  multi_funct mf2=std::bind
    (std::mem_fn<double(size_t,const ubvector &)>(&mcmc_para_new_class::f2),
     &mpc,std::placeholders::_1,std::placeholders::_2);
