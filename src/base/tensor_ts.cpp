...
int main(void) {
  ...
  for(size_t i=0;i<i1[0];i++) {
    j1[0]=i;
    arr.set(j1,((double)i));
    t.test_rel(arr.get(j1),i,1.0e-12,"R1 element.");
  }
  ...
  for(size_t i=0;i<i2[0];i++) {
    for(size_t j=0;j<i2[1];j++) {
      ...
      t.test_rel(mat.get(j2),i+j,1.0e-12,"R2 element.");
    }
  }
  ...
  for(size_t i=0;i<i3[0];i++) {
	...
	t.test_rel(m3.get(j3),i+j+k,1.0e-12,"R2 element.");
  ...
  for(size_t i=0;i<i3[0];i++) {
    for(size_t j=0;j<i3[1];j++) {
      j3[0]=i;
      j3[1]=j;
      ubvector_slice v=m3.vector_slice(2,j3);
      for(size_t k=0;k<i3[2];k++) {
	j3[2]=k;
	t.test_rel(m3.get(j3),v[k],1.0e-12,"vector slice 2.");
      }
    }
  }
  for(size_t i=0;i<i3[0];i++) {
    for(size_t k=0;k<i3[2];k++) {
      j3[0]=i;
      j3[2]=k;
      ubvector_slice v=m3.vector_slice(1,j3);
      for(size_t j=0;j<i3[1];j++) {
	j3[1]=j;
	t.test_rel(m3.get(j3),v[j],1.0e-12,"vector slice 1.");
      }
    }
  }
  for(size_t j=0;j<i3[1];j++) {
    for(size_t k=0;k<i3[2];k++) {
      j3[1]=j;
      j3[2]=k;
      ubvector_slice v=m3.vector_slice(0,j3);
      for(size_t i=0;i<i3[0];i++) {
	j3[0]=i;
	t.test_rel(m3.get(j3),v[i],1.0e-12,"vector slice 0.");
      }
    }
  }
  ...
  */
  ...
  t.test_gen(m3.get_size(0)==4,"size 1");
  t.test_gen(m3.get_size(1)==3,"size 2");
  t.test_gen(m3.get_size(2)==3,"size 3");
