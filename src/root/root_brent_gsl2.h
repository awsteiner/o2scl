...
namespace o2scl {
  ...
  */
  int iterate(func_t &f) {
    ...
    fp_t ab=b;
    if (ab<0.0) ab=-b;
    tol=ab*std::numeric_limits<fp_t>::epsilon()/two;
    m=(c-b)/two;
    ...
    fp_t ae=e, afa=fa, afb2=fb;
    if (ae<0.0) ae=-ae;
    if (afa<0.0) afa=-afa;
    if (afb2<0.0) afb2=-afb2;
	...
	p=-p;
      ...
      fp_t dtmp, atolq=tol*q;
      if (atolq<0.0) atolq=-atolq;
      fp_t aeq=e*q;
      if (aeq<0.0) aeq=-aeq;
  ...
  virtual int solve_bkt(fp_t &x1, fp_t x2, func_t &f) {
	...
	if (this->verbose>0) {
	  ...
	  fp_t delta=x_upper-x_lower;
	  if (delta<0.0) delta=-delta;
	  this->print_iter(root,y,iter,delta,this->tol_abs,
			   "root_brent_gsl (abs)");
	}
      }
	...
	if (ay<this->tol_rel) status=o2scl::success;
      
	if (this->verbose>0) {
	  this->print_iter(root,y,iter,ay,this->tol_rel,
			   "root_brent_gsl (rel)");
	}
      }
	...
	if (status==o2scl::success) {
	  ...
	  fp_t ay=y;
	  if (ay<0.0) ay=-ay;
	  if (ay>=this->tol_rel) status=gsl_continue;
	  if (this->verbose>0) {
	    this->print_iter(root,y,iter,ay,this->tol_rel,
			     "root_brent_gsl (rel)");
	  }
