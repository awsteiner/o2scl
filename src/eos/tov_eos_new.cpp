...
void tov_interp_eos::get_eden(double pres, double &ed, double &nb) {
  ...
  if (ldeos==true && pres<=prest/pwidth) {
    ...
    double edlo, edhi;
    double prlo=prest/pwidth;
    double prhi=prest*pwidth;

    // Low-density point
    interp((*ld_eos)[ldcolp],(*ld_eos)[ldcole],prlo,edlo,
	   0,ld_eos->get_nlines()-1);
    
    // High-density point
    interp((*eost)[colp],(*eost)[cole],prhi/pfactor,edhi,
	   0,eost->get_nlines()-1);
    edhi*=efactor;
    ...
    if (baryon_column) {
      double nblo, nbhi;
      // Low-density point
      interp((*ld_eos)[ldcolp],(*ld_eos)[ldcolnb],prlo,nblo,
	     0,ld_eos->get_nlines()-1);
      // High-density point
      interp((*eost)[colp],(*eost)[colnb],prhi/pfactor,nbhi,
	     0,eost->get_nlines()-1);
      nbhi*=nfactor;
