...
double eos_nstar_rot_interp::ed_from_nb(double nb) {
  return pow(10.0,interp(log_n_tab,log_e_tab,n_tab,log10(nb)));
}

double eos_nstar_rot_interp::nb_from_ed(double ed) {
  return pow(10.0,interp(log_e_tab,log_n_tab,n_tab,log10(ed)));
}
...
eos_nstar_rot_polytrope::eos_nstar_rot_polytrope(double index) {
  n_P=index;
  Gamma_P=1.0+1.0/n_P;
}

double eos_nstar_rot_polytrope::ed_from_pr(double pr) {
  return pr/(Gamma_P-1.0)+pow(pr,1.0/Gamma_P); 
}
