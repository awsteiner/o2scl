...
int eos_quark_njl::gap_func_msT(size_t nv, const ubvector &x, ubvector &y) {
  ...
  if (!std::isfinite(y[0]) || !std::isfinite(y[1]) ||
      !std::isfinite(y[2])) {
    cout << "Gap equation not finite (gap_func_msT)." << endl;
    exit(-1);
    return 2;
  }
