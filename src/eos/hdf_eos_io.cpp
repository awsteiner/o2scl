...
void o2scl_hdf::eos_had_strings_list() {
  ...
  list.push_back("skyrme t0[MeV] t1[MeV] t2[MeV] t3[MeV] x0 x1 x2 x3 alpha");
...
void o2scl_hdf::eos_had_temp_strings_list() {
...
eos_had_temp_base *o2scl_hdf::eos_had_temp_strings(std::string eos_str) {
  ...
  if (vs[0]=="skyrme" || vs[0]=="Skyrme") {
    ...
    if (vs.size()>=10) {
      ...
      ret=function_to_double_nothrow(vs[2],sk->t1,0,&r);
      if (ret!=0) return 0;
      sk->t1/=hc_mev_fm_f<double>();
      ret=function_to_double_nothrow(vs[3],sk->t2,0,&r);
      if (ret!=0) return 0;
      sk->t2/=hc_mev_fm_f<double>();
      ret=function_to_double_nothrow(vs[4],sk->t3,0,&r);
      if (ret!=0) return 0;
      sk->t3/=hc_mev_fm_f<double>();

      ret=function_to_double_nothrow(vs[5],sk->x0,0,&r);
      if (ret!=0) return 0;
      ret=function_to_double_nothrow(vs[6],sk->x1,0,&r);
      if (ret!=0) return 0;
      ret=function_to_double_nothrow(vs[7],sk->x2,0,&r);
      if (ret!=0) return 0;
      ret=function_to_double_nothrow(vs[8],sk->x3,0,&r);
      if (ret!=0) return 0;
    ...
    if (is_number(vs[1]) && vs.size()>=20) {
      ...
      rmf->a2=o2scl::stod(vs[12]);
      rmf->a3=o2scl::stod(vs[13]);
      rmf->a4=o2scl::stod(vs[14]);
      rmf->a5=o2scl::stod(vs[15]);
      rmf->a6=o2scl::stod(vs[16]);
      rmf->b1=o2scl::stod(vs[17]);
      rmf->b2=o2scl::stod(vs[18]);
      rmf->b3=o2scl::stod(vs[19]);
    } else {
      rmf_load(*rmf,vs[1]);
    }
    return rmf;
