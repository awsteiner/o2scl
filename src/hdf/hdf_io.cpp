...
void o2scl_hdf::hdf_output(o2scl_hdf::hdf_file &hf, tensor_grid &t,
			   std::string name) {
  ...
  const ubvector &d=t.get_data();
  hf.setd_vec_copy("data",d);
...
void o2scl_hdf::hdf_input(o2scl_hdf::hdf_file &hf, tensor_grid &t, 
			  std::string name) {
  ...
  if (name.length()==0) {
    ...
    if (name.length()==0) {
      O2SCL_ERR2("No object of type tensor found in ",
		 "tensor::hdf_input().",exc_efailed);
    }
  }
  ...
  hf.getd_vec_copy("data",t.get_data());
